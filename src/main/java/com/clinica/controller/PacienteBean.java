...
public void setPacientesFiltrados(List<Paciente> pacientesFiltrados) {
  ...
  public String guardar() {
    ...
    if (pacienteDAO.existe(paciente.getDpi())) {
        ...
        context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Paciente actualizado", "Se actualizaron los datos correctamente."));
    
    } else {
        pacienteDAO.guardar(paciente);
        context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Paciente guardado", "Se guardó el nuevo paciente."));
    }
